...
class SSD_Model:
    ...
    def fprop(self, input_data):
        ...
        self.interpreter.set_tensor(self.input_details[0]['index'], input_data.astype('float32'))
        self.interpreter.invoke()

        # update refs to output tensors
        self.boxes, self.classes, self.scores, self.num_detections = [
            self.interpreter.get_tensor(output['index']) for output in self.output_details
        ]
        # change int classes to their labels
        #self.classes = list(map(self.labels.get, self.classes))
        self.classes = np.array([self.labels[key] for key in self.classes.reshape([-1]).astype(int)])
    ...
    def get_output_tensors(self, threshold=None):
        ...
        if threshold is None:
            ...
            return self.boxes[idx], self.classes[idx], self.scores[idx], idx.size
...
if __name__ == '__main__':
    ...
    model = SSD_Model('./tf_files/detect.tflite', './tf_files/label_map.pbtxt')
